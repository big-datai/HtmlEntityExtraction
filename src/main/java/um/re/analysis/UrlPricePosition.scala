...
object UrlPricePosition {
 def main(args:Array[String]) {
    val conf = new SparkConf()
      .setAppName(getClass.getSimpleName)
     var (cassandraHost, keySpace, tableRT,tablePS, threshold ,path) = ("", "", "","","","")
    if (args.size == 4) {
    ...
    val seedPerDomain=realTime.orderBy(realTime("price")).groupBy("sys_prod_id").agg(max(realTime("sys_prod_id")) as "prodid", count(realTime("store_id")) as "SeedPerDom").cache
