  ...
  def main(args: Array[String]) {
    ...
    val deltaData = deltas.groupByKey(partitioner).flatMap {
      case (sys_prod_id, iter) =>
        ...
        val top2ByDelta = (sortedByDelta.reverse.head, "")
        //sortedByDelta.reverse.tail.head)
        val bottom2ByDelta = (sortedByDelta.head, "")
        //sortedByDelta.tail.head)
        val top2ByRelativeChange = (sortedByRelativeChange.reverse.head, "")
        //sortedByRelativeChange.reverse.tail.head)
        val bottom2ByRelativeChange = (sortedByRelativeChange.head, "")
        //sortedByRelativeChange.tail.head)
