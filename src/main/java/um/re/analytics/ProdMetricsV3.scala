...
object ProdMetricsV3 {
  ...
  def main(args: Array[String]) {
    ...
    if (args.size == 9) {
      ...
      tableRTM = "real_time_prod_metrics"
    ...
    try {
        ...
        case (sys_prod_id, iter) =>
          ...
          val results = stores.map {
            case (store_id, sys_prod_title) =>
              (sys_prod_id, store_id.replace(" ", ""), sys_prod_title, maxIncrease, maxIncreaseTo, maxIncreaseFrom, maxIncStoreId, maxIncProdId, maxDecrease, maxDecreaseTo, maxDecreaseFrom, maxDecStoreId, maxDecProdId)
          }
      ...
      deltaData.saveToCassandra(keySpace, tableRTM, SomeColumns("sys_prod_id", "store_id", "sys_prod_title", "max_increase", "max_increase_to", "max_increase_from", "max_inc_store_id",
        "max_inc_prod_id", "max_decrease", "max_decrease_to", "max_decrease_from", "max_dec_store_id", "max_dec_prod_id"))
...
//##########################Creating tables#################################
/*
CREATE TABLE demo.hotspot_prod_metrics (
    sys_prod_id text,
    store_id text,
    price double,
    url text,
...
CREATE TABLE demo.real_time_prod_metrics (
    sys_prod_id text,
    store_id text,
    sys_prod_title text,
    max_increase double,
