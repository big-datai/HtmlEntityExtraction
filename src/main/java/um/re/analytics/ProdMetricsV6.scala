      ...
      val deltas = hp.groupByKey(partitioner).map {
        case ((sys_prod_id, store_id), iter) =>
          ...
          if (iter.count(_ => true) > 1) {
            ...
            (sys_prod_id, (store_id, sys_prod_id, tmsp, price, sys_prod_title, delta, relativeChange,lastChange,lastChangeDte,lastPrice,prvPrice))
          } else {
            (sys_prod_id, (store_id, sys_prod_id, tmsp, price, sys_prod_title, 0.0, 0.0,0.0,tmsp,0.0,0.0))
          }
      }
      ...
      val deltaData = deltas.groupByKey(partitioner).flatMap {
        case (sys_prod_id, iter) =>
          ...
          val sortedByRelativeChange = sourceList.map {
            case (store_id, sys_prod_id, tmsp, price, sys_prod_title, delta, relativeChange,lastChange,lastChangeDte,lastPrice,prvPrice) =>
          ...
          val sortedByLastChange = sourceList.map {
            case (store_id, sys_prod_id, tmsp, price, sys_prod_title, delta, relativeChange,lastChange,lastChangeDte,lastPrice,prvPrice) =>
              (lastChange, (store_id, sys_prod_id,tmsp,sys_prod_title,lastChange,lastChangeDte,lastPrice,prvPrice))
          }
          ...
          val (maxIncLChange, maxIncLChangeTo, maxIncLChangeFrom, maxIncLChangeStoreId, maxIncLChangeProdId) = {
            ...
            if (increasesLChange.size > 0) {
              val forIncrease=increasesLChange.map{case (lastChange, (store_id, sys_prod_id, tmsp,sys_prod_title,lastChang,lastChangeDte,lastPrice,prvPrice)) =>
                val lstChangeTm = lastChangeDte
                lstChangeTm.setHours(0)
                lstChangeTm.setMinutes(0)
                lstChangeTm.setSeconds(0)
                (lstChangeTm, (store_id, sys_prod_id, tmsp,sys_prod_title,lastChang,lstChangeTm,lastPrice,prvPrice))}.sorted.reverse.head._1
                
                val increasesLChangeRevised=increasesLChange.map{case (lastChange, (store_id, sys_prod_id, tmsp,sys_prod_title,lastChang,lastChangeDte,lastPrice,prvPrice))=>
                val lstChangeTm = lastChangeDte
                lstChangeTm.setHours(0)
                lstChangeTm.setMinutes(0)
                lstChangeTm.setSeconds(0)
                (lastChange, (store_id, sys_prod_id, tmsp,sys_prod_title,lastChang,lstChangeTm,lastChangeDte,lastPrice,prvPrice))
              }

                
               val TopIncreasesLChange = increasesLChangeRevised.filter(l=>l._2._6==forIncrease).map{case(lastChange, (store_id, sys_prod_id, tm,sys_prod_title,lastChang,lstChangeTm,lastChangeDte,lastPrice,prvPrice))=>
                (lastChangeDte,(store_id, sys_prod_id,sys_prod_title,lastChang,lastPrice,prvPrice))}
               val temp = TopIncreasesLChange.sorted.reverse.head
               val (lastChange, (store_id, sys_prod_id, sys_prod_title,lastChang,lastPrice,prvPrice)) = (temp._2._4,(temp._2._1,temp._2._2,temp._2._3,temp._2._4,temp._2._5,temp._2._6))
               if (lastChange!=0.0){
          ...
          val (maxDecLChange, maxDecLChangeTo, maxDecLChangeFrom, maxDecLChangeStoreId, maxDecLChangeProdId) = {
            ...
            val forDecrease=decreasesLChange.map{case (lastChange, (store_id, sys_prod_id, tmsp,sys_prod_title,lastChang,lastChangeDte,lastPrice,prvPrice)) =>
                 val lstChangeTm = lastChangeDte
                lstChangeTm.setHours(0)
                lstChangeTm.setMinutes(0)
                lstChangeTm.setSeconds(0) 
            ...
            val decreasesLChangeRevised=decreasesLChange.map{case (lastChange, (store_id, sys_prod_id, tmsp,sys_prod_title,lastChang,lastChangeDte,lastPrice,prvPrice))=>
              val lstChangeTm = lastChangeDte
                lstChangeTm.setHours(0)
                lstChangeTm.setMinutes(0)
                lstChangeTm.setSeconds(0) 
            ...
            val TopDecreaseLChange = decreasesLChangeRevised.filter(l=>l._2._6==forDecrease).map{case(lastChange, (store_id, sys_prod_id, tm,sys_prod_title,lastChang,lstChangeTm,lastChangeDte,lastPrice,prvPrice))=>
                (lastChangeDte,(store_id, sys_prod_id,sys_prod_title,lastChang,lastPrice,prvPrice))}
             val temp = TopDecreaseLChange.sorted.reverse.head
             val (lastChange, (store_id, sys_prod_id, sys_prod_title,lastChang,lastPrice,prvPrice)) = (temp._2._4,(temp._2._1,temp._2._2,temp._2._3,temp._2._4,temp._2._5,temp._2._6))
             if (lastChange!=0.0){
                    (lastChange, lastPrice, prvPrice, store_id, sys_prod_id)}else{(0.0, 0.0, 0.0, "<>", "<>")}
            } else (0.0, 0.0, 0.0, "<>", "<>")
