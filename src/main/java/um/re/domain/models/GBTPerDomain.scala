...
object GBTPerDomain extends App {
  ...
  try {
    ...
    for (d <- list) {
      ...
      } catch { case e : Throwable => val errMsg = "model:  "+d+" "+e.getLocalizedMessage() +e.getMessage()
      sc.parallelize(List(errMsg), 1).saveAsTextFile(Utils.HDFSSTORAGE + Utils.DMODELS + "log/"+errMsg )  }
    }
  } catch { case e : Throwable => val errMsg = e.getLocalizedMessage() +e.getMessage() 
    sc.parallelize(List(errMsg), 1).saveAsTextFile(Utils.HDFSSTORAGE + Utils.DMODELS + "log/"+errMsg )  }
}