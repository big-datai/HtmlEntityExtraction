...
object BuildModels extends App {
  ...
  try {
    ...
    for (d <- parList) {
      try {
        ...
        val training_points = sc.objectFile[LabeledPoint]("/pard/" + dMap.apply(d) + "/training_points/", 10)
        val test_points = sc.objectFile[(String, LabeledPoint)]("/pard/" + dMap.apply(d) + "/test_points/", 10)
        ...
        try {
          ...
          sc.parallelize(scoreString, 1).saveAsTextFile("/pard/" + dMap.apply(d) + "/dscores/", classOf[GzipCodec]) // list on place i
          sc.parallelize(Seq(selectedModel)).saveAsObjectFile("/pard/" + dMap.apply(d) + "/selectedModel/")
          println("--------------------------------------------------------------------------------")
          println("--------------------------------------------------------------------------------")
          println("--------------------------------------------------------------------------------")
          ...
          println("--------------------------------------------------------------------------------")
          println("--------------------------------------------------------------------------------")
          sc.parallelize(scoreString, 1).saveAsTextFile(Utils.S3STORAGE + "/pard/" + dMap.apply(d) + "/dscores/", classOf[GzipCodec]) // list on place i
          sc.parallelize(Seq(selectedModel)).saveAsObjectFile(Utils.S3STORAGE + "/pard/" + dMap.apply(d) + "/selectedModel/")
        } catch {
