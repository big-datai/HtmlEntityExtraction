...
object RT2Report {
  def main(args: Array[String]) {
    ...
    try {
      ...
      lazy val cluster = Cluster.builder().addContactPoint(cassandraHost).build()
        ...
        val newPartition =
          x.map { l =>
            ...
            if (resultList.tail != Nil) {
              isMin = if (resultList.tail.head._1 < resultList.head._1) {
                0
              } else {
                1
              }
              isMax = if (resultList.tail.reverse.head._1 > resultList.head._1) {
                0
              } else {
                1
              }
