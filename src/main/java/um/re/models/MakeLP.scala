...
object MakeLP extends App {
  ...
  val all = source.map { l => (l._1.toString(), l._2.map { case (k, v) => (k.toString, v.toString()) }.toMap) }.repartition(80)
  ...
  val parsedData = all.map { l =>
    ...
    val parts_embedded = parts//parts.filter { w => (!w.isEmpty() && w.length > 2) }.map { w => w.toLowerCase }
